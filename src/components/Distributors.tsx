...
const WHATSAPP_NUMBER = "905355536884";

function buildWhatsAppUrl(shopName: string, phone: string, city: string, message: string) {
  const lines = [
...
export default function Distributors() {
  ...
  return (
    <section
      id="distributors"
      ref={ref}
      className="relative overflow-hidden bg-[var(--lime)] py-12 md:py-20"
    >
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-[1400px] px-6 md:px-10">
        <div className="grid gap-14 md:grid-cols-[1.1fr_1fr] md:gap-20">
          ...
          <div>
            ...
            <p className="dist-fade ar mt-8 max-w-lg text-lg leading-loose text-black/80 md:text-xl">
              إذا عندك سوبرماركت، محل، كافيه أو مطعم — EXTRON علامة جاهزة تعطيك
              منتج قوي، حضور بصري لافت، وطلب حقيقي من الزبائن.
            </p>
            <p className="dist-fade mt-2 text-sm uppercase tracking-[0.2em] text-black/50">
              Supermarkets · shops · cafés · restaurants
            </p>

            <ul className="dist-fade mt-8 grid gap-4 sm:grid-cols-2">
              {perks.map((p) => (
                <li
                  key={p.en}
                  className="perk-card border-2 border-black/20 bg-black/[0.04] p-5"
                >
